...
export const CheckboxGroupField: React.FC<CheckboxGroupFieldProps> = (props) => {
  const { useField, getEventValue, mounted, ref, setFieldRef, ...fieldContext } = useFieldContext();
  const { field, setFieldProps, setFieldHelpers } = useField<CheckboxGroupField>();
  ...
  const onValueChange = (value: any, checked: boolean) => {
    if (checked) {
      ...
      fieldContext.onValueChange(ref.current.value.filter?.((item: any) => value != item));
    }
  };
   ...
   */
  const isChecked = (value: any) => ref.current.value.some((item: any) => item == value);
